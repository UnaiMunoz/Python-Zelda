...
def move_character(map, position, direccion):
    ...
    if 0 <= new_position[0] < len(map) and 0 <= new_position[1] < len(map[0]):
        if map[new_position[0]][new_position[1]] not in ['#', 'O', 'T', 'C', 'S1?',  'S1 ', 'S0?', 'S0 ', 'M', 'F', '1', 'E9', '0', '~', '*','E1','t','W']:
    ...
    for i in range(-1, 2):
        for j in range(-1, 2):
            ...
            if 0 <= row < len(map) and 0 <= column < len(map[0]) and map[row][column] == 'C':
                ...
                if abrir.lower() == "yes":
                    objeto_obtenido = cofres_objetos.abrir_cofre("hyrule")
...
def obtener_espadamadera():
    ...
    UPDATE game_food
    SET quantity_remaining = quantity_remaining + 1
    WHERE food_name = 'wood sword' and game_id = (SELECT MAX(game_id) FROM game);""")
    conexion.commit()
...
def obtener_escudomadera():
    ...
    UPDATE game_food
    SET quantity_remaining = quantity_remaining + 1
    WHERE food_name = 'wood shield and game_id = (SELECT MAX(game_id) FROM game);""")
    conexion.commit()
...
map = [   ['*', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', 'O', 'O', 'O', '*', ' ', ' ', ' ', ' ', ' ', ' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ','*',],
          ['*', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', '~', 'O', 'O', '~', 'O', 'O', 'O', 'O', '~', '*',' ',f'{get_name()}',' ',' ',' ',' ','♥', f'{vida()}','/', f'{vida_total()} ',' ',' ','*',' ','',''],
...
while True:
    ...
    if user_input.lower() == "death mountain":
        ...
        actualizar_zona("Death Mountain")
        conexion.commit()
