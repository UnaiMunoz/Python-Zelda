     ...
     for i in range(-1, 2):
        for j in range(-1, 2):
            ...
            if 0 <= row < len(map) and 0 <= column < len(map[0]) and map[row][column] == 'C':
                ...
                interactuar_santuario5(map, character_position)
            elif 0 <= row < len(map) and 0 <= column < len(map[0]) and map[row][column] == 'S6?':
                interactuar_santuario6(map, character_position)

def open_chest(map, character_position):
    row, column = character_position

    # Verificar si hay un cofre en la posición del personaje
    if map[row][column] == 'M':
        open_command = input("¿Quieres abrir el cofre? (Escribe 'open chest'): ")
        
        if open_command.lower() == 'open chest':
            map[row][column] = 'W'  # Cambiar el símbolo del cofre a 'W'
            addText('¡Has abierto el cofre y has obtenido un escudo!')
        else:
            addText('Decides no abrir el cofre.')
...
def hit_tree(map, new_position):
    for i in range(-1, 2):
        for j in range(-1, 2):
            row = new_position[0] + i
            column = new_position[1] + j   
            if 0 <= row < len(map) and 0 <= column < len(map[0]) and map[row][column] == 'TT':
...
def interactuar_santuario5(map, character_position):
...
def interactuar_santuario6(map, character_position):
    global lives_character  # Declarar vida_personaje como global
...
while True:
    ...
    if direction.lower() == 'open chest':
        addText(direction)
        open_chest(map, character_position)
